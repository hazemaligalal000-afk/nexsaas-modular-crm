...
- [ ] 21. Implement Expense Management and Accounts Payable
  - [ ] 21.1 Create `expense_claims`, `purchase_orders`, `goods_receipts`, `supplier_invoices` table migrations
    - _Requirements: 20.1, 20.4_
  - [ ] 21.2 Implement expense claim submission with configurable approval workflow by amount threshold
    - Post journal entry on approval
    - _Requirements: 20.1, 20.2, 20.3_
  - [ ] 21.3 Implement three-way matching: PO + goods receipt + supplier invoice; flag discrepancy > 5%
    - _Requirements: 20.5, 20.6, 20.7_

- [ ] 22. Implement Inventory and Warehouse Management
  - [ ] 22.1 Create `inventory_items`, `warehouses`, `stock_ledger`, `inventory_stock` table migrations
    - _Requirements: 21.1, 21.2_
  - [ ] 22.2 Implement `StockMovementService::record()`: update quantities, append immutable stock ledger entry
    - Support batch/serial number tracking; FIFO/LIFO/weighted average valuation
    - _Requirements: 21.3, 21.5, 21.7_
  - [ ] 22.3 Implement reorder alert nightly task and optional auto-PO generation
    - _Requirements: 21.4_
  - [ ] 22.4 Implement stock take: lock items for counting, compute variance on completion
    - _Requirements: 21.6_

- [ ] 23. Implement Procurement Management
  - [ ] 23.1 Create `purchase_requisitions`, `rfqs`, `supplier_catalog` table migrations
    - _Requirements: 22.1, 22.2_
  - [ ] 23.2 Implement purchase requisition → approval → PO workflow
    - Track PO fulfillment status: pending, partially received, fully received, cancelled
    - _Requirements: 22.1, 22.3, 22.4_
  - [ ] 23.3 Implement RFQ: issue to multiple suppliers, record and compare quotes
    - Generate procurement spend report by supplier, category, period
    - _Requirements: 22.5, 22.6_

- [ ] 24. Implement HR and Employee Management
  - [ ] 24.1 Create `employees`, `departments`, `leave_types`, `leave_requests`, `employee_documents` table migrations
    - _Requirements: 23.1_
  - [ ] 24.2 Implement employee lifecycle management: onboarding, active, on leave, offboarding
  ...
  - [ ] 24.3 Implement leave management: configurable leave types, accrual rules, balance tracking, approval routing
    - _Requirements: 23.6, 23.7_
  - [ ] 24.4 Implement employee document storage with versioning, access restricted to HR role and above
    - _Requirements: 23.5_

- [ ] 25. Implement Payroll Processing (ERP)
  - [ ] 25.1 Create `payroll_runs` and `payroll_lines` table migrations with all 28 allowance + 18 deduction components
    - _Requirements: 24.1_
  - [ ] 25.2 Implement `PayrollRunService::compute()`: iterate active employees, apply salary components, deductions, tax tables; post journal entry
  ...
  - [ ] 25.3 Implement payroll export in bank transfer file formats (CSV, NACHA, BACS)
    - _Requirements: 24.5_
  - [ ] 25.4 Write property test for payroll negative net pay exclusion
    - **Property 25: Payroll Negative Net Pay Exclusion**
    - **Validates: Requirements 24.6, 52.13**

- [ ] 26. Implement Project Management
  - [ ] 26.1 Create `projects`, `project_tasks`, `milestones`, `time_logs` table migrations
    - _Requirements: 25.1_
  - [ ] 26.2 Implement project task hierarchy (up to 3 levels), dependencies (finish-to-start, start-to-start), conflict detection
    - Gantt chart data endpoint; completion percentage from completed/total tasks ratio
    - _Requirements: 25.2, 25.3, 25.4, 25.7_
  - [ ] 26.3 Implement milestone due-date notification and time tracking (actual vs. budgeted hours)
    - _Requirements: 25.5, 25.6_

- [ ] 27. Implement Manufacturing and Bill of Materials
  - [ ] 27.1 Create `boms`, `bom_lines`, `work_orders` table migrations
    - _Requirements: 26.1_
  - [ ] 27.2 Implement multi-level BOM with component quantity computation and stock availability check
    - On insufficient stock: list shortfall per component, optionally generate POs
    - _Requirements: 26.1, 26.2, 26.3, 26.4_
  - [ ] 27.3 Implement work order completion: consume components, add finished product to inventory, compute production cost
...
- [ ] 29. Implement COA Management (Batch A)
  - [ ] 29.1 Create `chart_of_accounts` migration with 5-level hierarchy, blocked flag, allowed_currencies, allowed_companies
    - _Requirements: 45.1, 45.2, 45.3, 45.4, 45.5_
  - [ ] 29.2 Implement `COAService` in `/modular_core/modules/Accounting/COA/`
  ...
  - [ ] 29.3 Implement allocation accounts engine: on posting to allocation account, distribute to target accounts by configured ratios
    - _Requirements: 45.11_
  - [ ] 29.4 Implement WIP stale check Celery task: flag WIP accounts with no movement > 90 days, notify Accountant
    - _Requirements: 45.12_

- [ ] 30. Implement Journal Entry and Voucher Engine (Batch B)
  - [ ] 30.1 Implement `VoucherService::save()` in `/modular_core/modules/Accounting/Vouchers/`
  ...
  - [ ] 30.2 Implement voucher approval state machine: Draft → Submitted → Approved → Posted → Reversed
  ...
  - [ ] 30.3 Implement FX rate auto-fill from Redis cache `fx:rate:{currency_code}:{date}` with user override
  ...
  - [ ] 30.4 Implement bulk voucher import from Excel: validate all 35 fields, report row-level errors before commit
    - _Requirements: 46.16_
  - [ ] 30.5 Implement voucher search interface filterable by Company_Code, Fin_Period, Voucher_Code, Section_Code, account_code, vendor_code, date range
    - _Requirements: 46.17_
  - [ ] 30.6 Implement bilingual PDF generation for posted vouchers (mPDF, Arabic RTL + English LTR, company letterhead)
    - _Requirements: 46.18, 58.6_
  - [ ] 30.7 Implement Settlement_Voucher engine: enforce Voucher_Code 999 with Section_Codes 991–996, calculate net settlement amount
    - _Requirements: 47.10_
  - [ ] 30.8 Implement multi-currency trial balance showing debit/credit/net in transaction currency and EGP per Company_Code per Fin_Period
    - _Requirements: 47.11_

- [ ] 31. Implement Multi-Currency and Exchange Rate Engine (Batch C)
  - [ ] 31.1 Create `exchange_rates` table migration; implement currency master UI with 6 currencies
    - _Requirements: 47.1, 47.2_
  - [ ] 31.2 Implement `FXService` in `/modular_core/modules/Accounting/FX/`
  ...
  - [ ] 31.3 Implement `FXService::computeRealizedGainLoss()`: post gain/loss to configured FX account on settlement
    - _Requirements: 47.6_
  - [ ] 31.4 Implement `FXRevaluationTask` (Celery): period-close unrealized revaluation + auto-reversal on first day of next period
    - _Requirements: 47.7, 47.8_
  - [ ] 31.5 Implement currency translation report (foreign currency → EGP using closing rate method)
    - _Requirements: 47.9_
  - [ ] 31.6 Write property test for realized FX gain/loss calculation
    - **Property 23: Realized FX Gain/Loss Calculation**
    - **Validates: Requirements 47.6**
  - [ ] 31.7 Write property test for FX revaluation round trip
    - **Property 24: FX Revaluation Round Trip**
    - **Validates: Requirements 47.7, 47.8**

- [ ] 32. Implement Accounts Receivable and Payable (Batch D)
  - [ ] 32.1 Create `ar_invoices`, `ap_bills`, `payments`, `payment_allocations`, `accruals` table migrations
    - _Requirements: 48.1, 48.2_
  - [ ] 32.2 Implement AR invoice creation and AP bill creation with payment matching (partial and full allocation)
  ...
  - [ ] 32.3 Implement AR and AP aging reports: 0–30, 31–60, 61–90, 91–120, 120+ day buckets per vendor per currency per Company_Code
    - _Requirements: 48.6, 48.7_
  - [ ] 32.4 Implement withholding tax auto-deduction on vendor payment disbursement
    - _Requirements: 48.8_
  - [ ] 32.5 Implement retention tracking per contract; release on milestone completion
    - _Requirements: 48.9_
  - [ ] 32.6 Implement accruals module: auto-reverse accrual entry at start of next Fin_Period
    - _Requirements: 48.10_
  - [ ] 32.7 Implement sister-company intercompany reconciliation view (net AR/AP between each pair of 6 companies)
    - _Requirements: 48.11_
  - [ ] 32.8 Implement partner dues and withdrawals tracking per Partner_Code per Company_Code
    - _Requirements: 48.12_
  - [ ] 32.9 Implement customer statement (all transactions, payments, running balance per vendor per Fin_Period)
    - _Requirements: 48.13_
  - [ ] 32.10 Implement overdue AR alert Celery task; implement ETA e-invoice formatting, signing, and submission for Company 01
    - _Requirements: 48.14, 48.15_

- [ ] 33. Implement Bank and Cash Management (Batch E)
  - [ ] 33.1 Create `bank_accounts`, `petty_cash_funds`, `cash_calls`, `bank_transactions` table migrations
    - _Requirements: 49.1_
  - [ ] 33.2 Implement bank account master covering all configured accounts (SAIB LE/EGP, PAYPALL USD, AMARAT AED, QNB E-WALLET, INSTAPAY NBE)
  ...
  - [ ] 33.3 Implement bank statement CSV import with voucher matching and unmatched item highlighting
  ...
  - [ ] 33.4 Implement PAYPALL USD balance with EGP equivalent; mobile wallet tracker for QNB E-WALLET and INSTAPAY
    - Cash position dashboard: all bank/cash balances per currency with total EGP equivalent
    - _Requirements: 49.8, 49.9, 49.10_
  - [ ] 33.5 Implement cash flow forecast (30/60/90 days) from open AR/AP per Company_Code
    - Month-end bank interest accrual calculation and journal entry posting
    - _Requirements: 49.11, 49.12_

- [ ] 34. Implement Cost Centers and Project Accounting (Batch F)
  - [ ] 34.1 Create `cost_centers`, `cost_center_budgets`, `afes` table migrations
    - _Requirements: 50.1, 50.2_
  - [ ] 34.2 Implement cost center master with full hierarchy linked to Company_Code
    - Annual budgets per cost center per expense account; budget vs. actual report with drill-down
    - _Requirements: 50.1, 50.2, 50.3_
  - [ ] 34.3 Implement cost allocation engine: distribute indirect expenses from allocation account to target cost centers by configured ratios
    - _Requirements: 50.4_
  - [ ] 34.4 Implement WIP tracking per AFE number: WIP EXP, WIP DEV, WIP CONSTR accounts
  ...
  - [ ] 34.5 Implement department time allocation: distribute payroll costs across cost centers on payroll journal post
...
- [ ] 35. Implement Fixed Assets (Batch G)
  - [ ] 35.1 Create `fixed_assets` table migration with all asset categories and depreciation fields
    - _Requirements: 51.1_
  - [ ] 35.2 Implement asset acquisition linking to tangible cost COA account
    - Depreciation setup per category: straight-line or declining balance, useful life, salvage value
    - _Requirements: 51.2, 51.3_
  - [ ] 35.3 Implement monthly depreciation Celery task: calculate and post depreciation journal entries for all active assets per Company_Code
    - _Requirements: 51.4_
  - [ ] 35.4 Implement asset disposal: calculate gain/loss vs. net book value, post disposal journal entry to RETIRED ASSETS & EQUIPMENT and ASSETS CLEARING ACCOUNT, mark retired
    - Salvage material gain/loss posting
    - _Requirements: 51.5, 51.10_
  - [ ] 35.5 Implement asset revaluation: post revaluation difference to equity revaluation reserve
    - Asset overhaul/CAPEX: capitalize or expense based on configurable threshold
    - _Requirements: 51.6, 51.7_
  - [ ] 35.6 Implement asset register report (cost, accumulated depreciation, net book value per asset per Company_Code)
    - Asset movement report (inter-company transfers)
    - _Requirements: 51.8, 51.9_

- [ ] 36. Implement Payroll and Salary Module (Batch H)
  - [ ] 36.1 Create `payroll_runs` and `payroll_lines` table migrations with all 28 allowance + 18 deduction components per design schema
    - _Requirements: 52.1, 52.2_
  - [ ] 36.2 Implement `PayrollRunService::compute()` for Accounting module
  ...
  - [ ] 36.3 Implement bilingual payslip PDF (Arabic + English) per employee per Payroll_Run
    - ATM salary payment file in configurable bank transfer format
    - _Requirements: 52.5, 52.6_
  - [ ] 36.4 Implement on-loan employee tracking: post salary cost to Loanes Sal. From Other / To Other / Onloan Epsco accounts
    - Board member compensation run with separate journal entry
    - _Requirements: 52.7, 52.8_
  - [ ] 36.5 Implement social insurance report (employer + employee share per employee per month)
...
- [ ] 37. Implement Partner Profit Distribution (Batch I)
  - [ ] 37.1 Create `partners` table migration with `share_pct` and `withdrawal_approval_threshold`
    - _Requirements: 53.1_
  - [ ] 37.2 Implement partner distribution on Fin_Period close: aggregate net income, calculate each partner's share, post distribution journal entry (debit ANNUAL PROFIT, credit PARTNER DUES)
    - _Requirements: 53.1, 53.2_
  - [ ] 37.3 Implement partner withdrawal: post journal entry (debit PARTNER DUES, credit bank); require dual approval when amount exceeds threshold
    - Track partner capital in SHARE CAPITAL account per Company_Code
    - _Requirements: 53.3, 53.4, 53.7_
  - [ ] 37.4 Implement partner account statement (dues, withdrawals, running balance per Partner_Code per Company_Code per Fin_Period)
  ...
  - [ ] 37.5 Write property test for partner profit distribution calculation
    - **Property 26: Partner Profit Distribution Calculation**
    - **Validates: Requirements 53.1**

- [ ] 38. Implement Financial Statements (Batch J)
  - [ ] 38.1 Implement Trial Balance per Company_Code per Fin_Period (debit, credit, net per account), exportable to Excel
    - _Requirements: 54.1_
  - [ ] 38.2 Implement auto-generated P&L Statement (Income − Cost − Expenses per Company_Code per Fin_Period)
    - Balance Sheet verifying Assets = Liabilities + Equity with comparative prior-period columns
    - _Requirements: 54.2, 54.3_
  - [ ] 38.3 Implement Cash Flow Statement (direct method from bank account movements)
    - Consolidated financial statements aggregating all 6 companies with inter-company elimination
    - _Requirements: 54.4, 54.5_
  - [ ] 38.4 Implement department P&L by cost center using time-allocation entries
  ...
  - [ ] 38.5 Implement financial period close checklist: reconcile AR/AP, revalue FX, post depreciation, allocate indirect expenses, post partner profit — each step with completion status
    - Immutable audit trail report (user, timestamp, IP, Company_Code, Fin_Period), exportable as PDF
    - _Requirements: 54.9, 54.10_

- [ ] 39. Implement Tax and Compliance (Batch K)
  - [ ] 39.1 Implement withholding tax ledger with monthly reconciliation report
    - Monthly income tax provision calculation and journal entry posting
    - _Requirements: 55.1, 55.2_
  - [ ] 39.2 Implement social insurance Form 2 report per employee per Company_Code
    - VAT tagging on invoices and VAT report per Fin_Period
    - _Requirements: 55.3, 55.4_
  - [ ] 39.3 Implement ETA e-invoice: format as JSON per ETA schema, sign payload, submit to ETA API, store submission status and response (Company 01 only)
    - _Requirements: 55.5_
  - [ ] 39.4 Implement tax card and commercial registry expiry alerts (90/60/30 days) to Owner and Admin
...
- [ ] 40. Implement Accounting Reporting and Analytics Dashboard (Batch L)
  - [ ] 40.1 Implement accounting home dashboard: cash position, AR/AP aging summary, pending approval count, period close checklist progress
    - _Requirements: 56.1_
  - [ ] 40.2 Implement voucher volume bar chart, currency exposure donut chart, partner equity dashboard
    - Cost center spend heatmap (actual vs. budget, color-coded by variance %)
    - _Requirements: 56.2, 56.3, 56.4, 56.5_
  - [ ] 40.3 Implement top expense accounts ranked bar chart, inter-company balances monitor (sum must equal zero), cash call tracker widget, WIP account balances widget
    - _Requirements: 56.6, 56.7, 56.8, 56.9_
  - [ ] 40.4 Implement custom report builder: select journal entry line fields, group-by, filter, sort; export as CSV, Excel, PDF
    - Scheduled reports: auto-generate and email Trial Balance, P&L, Cash Position at month-end
    - _Requirements: 56.10, 56.11_

- [ ] 41. Implement Platform-Wide Accounting Features (Batch N)
  - [ ] 41.1 Implement company switcher in top navigation bar; re-scope all accounting queries on company change
    - _Requirements: 58.1_
  - [ ] 41.2 Implement financial period manager: open, close, lock YYYYMM periods per Company_Code; prevent backdated posting to locked period
    - _Requirements: 58.2_
  - [ ] 41.3 Implement multi-company journal entries: intercompany flag posts lines to each involved Company_Code atomically
    - _Requirements: 58.3_
  - [ ] 41.4 Implement accounting audit log UI filterable by User, Company_Code, account_code, Fin_Period, action type; immutable and exportable
    - _Requirements: 58.4_
  - [ ] 41.5 Enforce accounting RBAC permissions: accounting.voucher.create, accounting.voucher.approve, accounting.voucher.reverse, accounting.period.close, accounting.statements.view, accounting.payroll.run, accounting.partner.distribute
    - _Requirements: 58.5_
  - [ ] 41.6 Implement bilingual PDF engine for all accounting reports and vouchers (Arabic RTL + English LTR sections)
    - Format monetary amounts as #,##0.00 with currency symbol; Arabic-Indic numerals for Arabic locale
    - _Requirements: 58.6, 58.7_
  - [ ] 41.7 Implement full accounting data export per Company_Code per fiscal year as Excel workbook (one sheet per module)
    - _Requirements: 58.8_

- [ ] 42. Checkpoint — Ensure all Accounting tests pass
  - Ensure all tests pass, ask the user if questions arise.
...
- [ ] 43. Implement WebSocket Notification Server
  - [ ] 43.1 Implement Ratchet/Swoole WebSocket server with per-user authenticated channels
  ...
  - [ ] 43.2 Implement `notifications` table migration and 90-day TTL nightly Celery cleanup task
  ...
  - [ ] 43.3 Implement `useNotifications()` React hook maintaining WS connection and dispatching to notification store
    - _Requirements: 27.1, 27.2_

- [ ] 44. Implement SaaS Subscription Billing via Stripe
  - [ ] 44.1 Implement Stripe Billing integration: create Stripe Customer on tenant signup, attach payment method
    - _Requirements: 28.1, 28.2_
  - [ ] 44.2 Implement Stripe webhook handler: update plan status to active within 60s on `invoice.payment_succeeded`
    - Notify Owner and restrict paid features after 7-day grace period on payment failure
    - _Requirements: 28.3, 28.4_
  - [ ] 44.3 Implement subscription management UI: upgrade, downgrade, cancel plan
...
- [ ] 45. Implement Global Search
  - [ ] 45.1 Implement `GlobalSearchService` combining PostgreSQL full-text search and pgvector semantic search
  ...
  - [ ] 45.2 Implement `GET /api/v1/platform/search` endpoint and global search bar React component
    - _Requirements: 29.1, 29.2_

- [ ] 46. Implement Audit Logging
  - [ ] 46.1 Implement append-only `audit_log` table with PostgreSQL row-level security preventing UPDATE/DELETE
  ...
  - [ ] 46.2 Implement audit log search UI filterable by user, date range, record type, operation type; return within 5s for 10M entries
    - _Requirements: 30.4, 30.5_

- [ ] 47. Implement Webhook Management
  - [ ] 47.1 Create `webhooks` and `webhook_deliveries` table migrations
    - _Requirements: 31.1_
  - [ ] 47.2 Implement webhook registration, HMAC-SHA256 payload signing, and HTTP POST delivery within 10s
...
- [ ] 48. Implement Internationalization (i18n)
  - [ ] 48.1 Implement `react-i18next` with locale files per module; RTL layout via `dir="rtl"` on `<html>` for Arabic
    - Externalize all user-facing strings; fall back to English on missing translation
    - _Requirements: 32.1, 32.2, 32.3, 32.4, 32.5_
  - [ ] 48.2 Implement Smarty bilingual templates for PHP-rendered views (Arabic RTL + English LTR)
    - _Requirements: 32.4, 58.6_

- [ ] 49. Checkpoint — Ensure all Platform Core tests pass
  - Ensure all tests pass, ask the user if questions arise.
...
- [ ] 50. Implement AI Engine FastAPI application structure
  - [ ] 50.1 Set up FastAPI app in `/ai_engine/app/` with standard response schema `{ result, confidence, model_version }`
  ...
  - [ ] 50.2 Set up Celery workers in `/ai_engine/workers/` consuming RabbitMQ queues
    - _Requirements: 35.1_

- [ ] 51. Implement Churn Prediction
  - [ ] 51.1 Implement `ChurnPredictionService` (survival analysis on engagement, recency, support volume, contract renewal proximity)
  ...
  - [ ] 51.2 Implement high-churn task creation for Account owner and daily recompute Celery task for all Accounts
    - _Requirements: 36.4, 36.5_

- [ ] 52. Implement NLP and Sentiment Analysis
  - [ ] 52.1 Implement `SentimentService` (multilingual BERT for Arabic + English)
    - FastAPI endpoint `POST /predict/sentiment`; store sentiment label + confidence on inbox_messages
    - _Requirements: 37.1, 37.2_
  - [ ] 52.2 Implement negative sentiment flagging (confidence > 0.75) for supervisor review
    - Display sentiment indicators on conversation list; sentiment trend report per Account
    - _Requirements: 37.3, 37.4, 37.5_

- [ ] 53. Implement AI-Powered Semantic Search with Embeddings
  - [ ] 53.1 Create `record_embeddings` table migration with pgvector extension and ivfflat index
    - _Requirements: 38.1, 38.2_
  - [ ] 53.2 Implement `EmbeddingService` (sentence-transformers) generating 768-dim vectors per record
  ...
  - [ ] 53.3 Combine semantic + keyword search results, deduplicate before returning
    - _Requirements: 38.4_

- [ ] 54. Implement AI Email and Response Suggestions
  - [ ] 54.1 Implement FastAPI endpoint `POST /predict/email-suggestions`
  ...
  - [ ] 54.2 Implement suggestion display in reply composer; log original + edited text for model improvement on edit
    - _Requirements: 39.2, 39.3, 39.5_

- [ ] 55. Implement Revenue Forecasting and Intelligence
  - [ ] 55.1 Implement `ForecastService` (ARIMA/Prophet) for monthly revenue forecast (next 3 months) with confidence intervals
    - FastAPI endpoints `POST /predict/revenue-forecast` and `POST /predict/cash-flow-forecast`
    - _Requirements: 40.1, 40.3_
  - [ ] 55.2 Display AI forecast alongside weighted pipeline forecast on revenue dashboard
    - Daily refresh via Celery queue; track forecast accuracy on month close
    - _Requirements: 40.2, 40.4, 40.5_

- [ ] 56. Implement AI Accounting Features (Batch M)
  - [ ] 56.1 Implement exchange rate deviation check: compare entered rate to Redis-cached market rate; flag if deviation exceeds threshold
    - FastAPI endpoint `POST /accounting/anomaly-detect`
    - _Requirements: 57.1_
  - [ ] 56.2 Implement duplicate voucher check: search posted vouchers by vendor + amount + date ±3 days; warn Accountant
    - FastAPI endpoint `POST /accounting/duplicate-check`
    - _Requirements: 57.2_
  - [ ] 56.3 Implement account code suggestion from cost_identifier free-text using text similarity (top 3 COA matches)
    - FastAPI endpoint `POST /accounting/account-suggest`
    - _Requirements: 57.3_
  - [ ] 56.4 Implement WIP stale balance AI flagging in monthly Celery task; notify Accountant
    - _Requirements: 57.4_
  - [ ] 56.5 Implement cash flow prediction (next 3 months per Company_Code per currency) displayed on cash position dashboard
    - Translation revenue forecast: vendor_word_count × per-word rate aggregated by Fin_Period
    - _Requirements: 57.5, 57.6_
  - [ ] 56.6 Implement journal entry line amount outlier detection: compare Dr/Cr to historical entries on same account; flag statistical outliers
    - _Requirements: 57.7_

- [ ] 57. Checkpoint — Ensure all AI Engine tests pass
  - Ensure all tests pass, ask the user if questions arise.
...
- [ ] 58. Implement Performance, Caching, and Scalability
  - [ ] 58.1 Implement Redis caching for permissions, tenant config, and lookup tables with appropriate TTLs
    - Fall back to DB reads when Redis unavailable; log cache unavailability warning
    - _Requirements: 41.3, 41.4_
  - [ ] 58.2 Implement database connection pooling (min 10, max 100 connections per app instance)
    - _Requirements: 41.5_
  - [ ] 58.3 Write Kubernetes HPA manifests for PHP backend and AI Engine pod autoscaling
    - _Requirements: 41.2_

- [ ] 59. Implement Security Hardening
  - [ ] 59.1 Implement input sanitization middleware for SQL injection and XSS prevention on all endpoints
    - CSRF protection on all state-changing HTTP endpoints
    - Enforce HTTPS via HSTS header
    - _Requirements: 42.2, 42.3, 42.4_
  - [ ] 59.2 Implement request body JSON Schema validation returning HTTP 422 with field-level error map on failure
    - Implement Zod schema validation on React frontend before submission
    - _Requirements: 44.2, 44.3_

- [ ] 60. Implement API Serializer Integrity
  - [ ] 60.1 Implement consistent UTF-8 JSON serialization with consistent key ordering and no trailing whitespace
    - Represent monetary amounts as strings or fixed-point decimals in JSON (never float)
    - _Requirements: 44.1, 44.4, 44.6_
  - [ ] 60.2 Write property test for API serialization round trip
    - **Property 22: API Serialization Round Trip**
    - **Validates: Requirements 44.5**
  - [ ] 60.3 Write property test for standard table schema invariant
    - **Property 3: Standard Table Schema Invariant**
    - **Validates: Requirements 1.1, 1.2**

- [ ] 61. Implement Data Export and Portability
  - [ ] 61.1 Implement full tenant data export (JSON + CSV) via async Celery job; notify Owner when ready
    - Complete within 4 hours for up to 1M records; download available for 7 days
    - _Requirements: 43.1, 43.2, 43.3, 43.4_
  - [ ] 61.2 Implement right-to-erasure: permanently remove all tenant records and revoke all active sessions within 24 hours
    - _Requirements: 43.5_

- [ ] 62. Implement React Frontend Core
  - [ ] 62.1 Set up React 18/Vite project in `/frontend/src/` with module structure: `components/`, `modules/`, `hooks/`, `lib/`
    - Implement `PermissionGate` component wrapping UI elements based on resolved permissions
    - _Requirements: 2.4, 2.5_
  - [ ] 62.2 Implement React Query hooks per module (`useContacts`, `useDeals`, `useVouchers`, etc.)
    - Implement typed API client functions with standard envelope parsing
    - _Requirements: 3.1, 3.2, 3.3_
  - [ ] 62.3 Implement SOAP legacy endpoints in `/soap/` and `/webservice.php` enforcing tenant_id and RBAC identically to REST
    - _Requirements: 5.1_

- [ ] 63. Final Integration and Wiring
  - [ ] 63.1 Wire all RabbitMQ event bus integrations per the integration patterns table
  ...
  - [ ] 63.2 Wire all Celery scheduled tasks: deal rotting, overdue check, FX rate refresh, FX revaluation, depreciation, payroll, embedding updates, notification cleanup, churn recompute, forecast refresh, WIP stale check, recurring invoices, AR overdue alerts
    - _Requirements: 10.8, 19.6, 36.5, 47.5, 47.7, 51.4, 38.5, 27.5, 40.4_
  - [ ] 63.3 Wire AI Engine PHP → Python internal API calls for all prediction endpoints
